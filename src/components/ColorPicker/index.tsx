    ...
    <div className="flex gap-4">
        
        {
            colors.map( (currentColor, index) => {               
                ...
                return   (
                        <label key={index} htmlFor={index.toString()} style={{ backgroundColor:currentColor }} className="relative size-12 rounded-full hover:cursor-pointer">
                            <input className="peer hidden" type="radio" name="color" id={index.toString()} value={currentColor} onChange={handleRadioChange} checked={currentColor === color} />
