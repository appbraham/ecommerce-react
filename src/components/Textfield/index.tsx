...
const Textfield = ({inputType, label, icon="search"}: ITextfield) => {
  ...
  const renderIcon = () => {
    ...
    // return <svg className="absolute -translate-y-1/2 top-1/2 left-4" width="1em" height="1em" viewBox="0 0 24 24"><path className="stroke-black/50" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="m21 21-4.343-4.343m0 0A8 8 0 1 0 5.343 5.343a8 8 0 0 0 11.314 11.314"/></svg>;
  }

  return (
    <div className="relative w-full hidden lg:block">
        <input type={inputType} placeholder={label} className="w-full h-12 font-satoshi font-medium text-base bg-gray-200 rounded-full pl-12 pr-6 py-2 placeholder:text-black/50" />
        {renderIcon()}
    </div>
  );
}
